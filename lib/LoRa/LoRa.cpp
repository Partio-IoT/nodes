...
void LoRaClass::setTxPower(int level, int outputPin)
 {
   if (PA_OUTPUT_RFO_PIN == outputPin) {
    ...
    if (level > 17) {
      ...
      writeRegister(REG_PA_DAC, 0x87); //Sallitaan 20 dBm
	setOCP_sx127x(140);	//Muutetaan virranrajoitusta
	writeRegister(REG_PA_CONFIG, 0xFF); //Asetetaan 11111111 	//RegPaConfig eli maksimi 20 dBm teho 
}
    else {
      if (level < 2) {
        level = 2;
